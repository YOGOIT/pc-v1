<template>
  <div class="content">
    <div class="box">
      <nav-member v-if="renderComponent" :cid="17"></nav-member>
      <div class="messages-box">
        <div class="btn-read" @click="readAll()">全部证书</div>
        <!-- <span>{{ data }}----------{{list}}</span> -->
        ...
        <div>
          <ul>
            <li v-for="item in list" :key="item">
              <img src="https://api.camec.sdyougao.com/storage/images/mO1QPZZ4VsoVjGN2eK8wznrobr3Ju92Q84e4UpJX.png" alt="">
              {{ item.content }}
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true;
      this.$api.Member.Messages2(this.pagination).then((res) => {
        this.loading = false;
        this.data = res.data;
        this.list = res.data;
      });
